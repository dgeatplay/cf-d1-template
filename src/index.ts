	...
	async fetch(request, env) {
		...
		if (url.pathname === "/snow") {
			const stmt = env.DB.prepare(`
				SELECT display_at, temp, pop, precip_accum, precip_snow 
				FROM hourly_forecasts 
				WHERE location_id = '141' 
				ORDER BY display_at ASC 
				LIMIT 120
			`);
			const { results } = await stmt.all<{ display_at: string; temp: number; pop: number; precip_accum: number; precip_snow: number }>();
